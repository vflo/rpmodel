...
calc_ga <- function(u,ustar,R,tc,p){
  ...
  if(!is.na(ustar)){
    (1/((u/ustar)+135*ustar^(-0.67)))*p/(R*(tc+273.15)) #Thom 1972 Momentum, mass and heat exchange of vegetation
  }else{
    (u/208)*p/(R*(tc+273.15)) #Allen et al 1998 crop evapotranspiration-guidelines for computing crop water requirements
  }
}
...
calc_gs_PM = function(dpsi, psi_soil, par_plant, par_env, PM_params, ...){
  ...
  divid <- C*(S*Q+dens*cp*vpd*ga*R*(tc+273.15)/patm)/(L*(-K*integral_P(dpsi, psi_soil, par_plant$psi50, par_plant$b, ...))) -S -pch

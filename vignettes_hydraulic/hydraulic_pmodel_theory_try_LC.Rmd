...
```{r}
fn_profit = function(x, psi_soil, par_env, par_photosynth, par_plant){
  ...
  cA = 1.6*D/ca/(1-x)
  cB = 146 * (x*ca+kmm)/(x*ca-g)
  cC = 40 * (1.6*D/(ca*(1-x)*K*Pox))^2*A
  
  # cat(cA, cB, cC, "\n")
  cA+cB+cC
}

fn_profit(.5, psi_soil = 0, par_photosynth = get_std_photosythnesis_params(), par_plant = par_plant_std, par_env = par_env_std)
...
dat <- list(x=seq(0.15,0.95, length.out=50), psis=c(-2,-1.5,-1,-0.5,0)) %>% cross_df() %>% 
  mutate(profit = purrr::map2_dbl(x, psis, ~fn_profit(.x, psi_soil = .y, par_photosynth = get_std_photosythnesis_params(), par_plant = par_plant_std, par_env = par_env_std)))
...
```{r}
optimize_midterm_multi <- function(fn_profit, psi_soil, par_photosynth, par_plant, par_env, return_all = FALSE){
  
  out_optim <- optimr::optimr(
    par       = c(x=0.75),  
    lower     = par_photosynth$gammastar/par_photosynth$ca+0.001,
    upper     = 1,
    fn        = fn_profit,
    psi_soil  = psi_soil,
...
datopt <- tibble(psis=c(-2,-1.5,-1,-0.5,0)) %>% 
  mutate(x = purrr::map(psis, ~optimize_midterm_multi(fn_profit, psi_soil = ., par_photosynth = get_std_photosythnesis_params(), par_plant = par_plant_std, par_env = par_env_std, return_all = T))) %>% unnest_wider(x)
